...
class Kernels:
    ...
    """
    def __init__(self, vza, sza, raa, critical=1, RossHS=False, RecipFlag=True, HB=2.0, BR=1.0,
                 MODISSPARSE=True, MODISDENSE=False, RossType='Thick',
                 normalise=1, normalize=0, LiType='Sparse', doIntegrals=False, BSAangles=[], nbar=0.0):
        """
        ...
        The kernels are accessible from:
            ...
            self.Ross
            self.Li
        The angles are accessible from:
        ...
        Kernels integrals are accessible from:
        ...
        If you want to mimic the results in Wanner et al. 1995, I've set a special function called self.mimic
        at the end here. This requires pylab to plot the data.

        Example of use:
            see mimic()

        """
        self.__setup(critical=critical, RecipFlag=RecipFlag, RossHS=RossHS, HB=HB, BR=BR,
                     MODISSPARSE=MODISSPARSE, MODISDENSE=MODISDENSE, RossType=RossType,
                     normalise=normalise, normalize=normalize, LiType=LiType,
                     doIntegrals=doIntegrals, BSAangles=BSAangles, nbar=nbar)
        self.setAngleInfo(vza, sza, raa)
        self.__doKernels()
        self.__postProcess()

    def __setup(self, critical=1, RecipFlag=True, RossHS=False, HB=2.0, BR=1.0,
                MODISSPARSE=True, MODISDENSE=False, RossType='Thick',
                normalise=1, normalize=0, LiType='Sparse', doIntegrals=True, BSAangles=[], nbar=0.0):
        self.nbar = nbar
    ...
    def __postProcess(self):
        ...
        # if we are normalising the last element of self.Isotropic, self.Ross and self.Li
        # contain the nadir-nadir kernel
        if self.normalise >= 1:
    ...
    def setAngleInfo(self,vza,sza,raa):
        ...
        if self.N != len(self.szaDegrees) or self.N != len(self.raaDegrees):
            self.error('kernels: inconsistent number of samples in vza, sza and raa data: ' +
                       str(len(self.vzaDegrees)) + ', ' + str(len(self.szaDegrees)) + ', '
                       + str(len(self.raaDegrees)), critical=self.critical)
    ...
    def __integrateKernels(self,BSAangles=[]):
        ...
        @option: BSAangles=[] allows the user to set the sza angles at which directional-hemispherical integral
        is calculated, else steps of 1 degree from 0 to 89 (though I wouldn't trust it down to 90)
        This function can be rather slow, so using fewer samples or an approximate function may be a good idea.
        """
        ...
        try:
            ...
            for sza in self.BSAangles:
                # ross integral
                # self.BSA_Ross[i], self.BSA_Ross_error[i] = scipy.integrate.dblquad(RossFunctionForIntegral,
                #                                                                    0.0, 1.0, __gfun, __hfun,
                #                                                                    args=(sza,self))
                # self.BSA_Li[i], self.BSA_Li_error[i] = scipy.integrate.dblquad(LiFunctionForIntegral,
                #                                                                0.0, 1.0, __gfun, __hfun,
                #                                                                args=(sza,self))
                i += 1
    ...
    def GetOverlap(self):
        ...
        self.__cost =  self.HB * numpy.sqrt(self.__distance * self.__distance
                                            + self.__tan1 * self.__tan1 * self.__tan2
                                            * self.__tan2 * self.__sin3 * self.__sin3) / self.__temp;
    ...
    def printIntegrals(self, header=True, reflectance=False):
        ...
        """
        if header:
            self.printer('# ' + str(self.N) + ' samples Ross: ' + self.RossType +
                         ' Li: ' + self.LiType + ' Reciprocal: ' + str(self.RecipFlag) +
                         ' normalisation: ' + str(self.normalise) +
                         ' HB ' + str(self.HB) + ' BR ' + str(self.BR) + '\n')
    ...
    def printKernels(self, header=True, reflectance=False, file_=False):
        """
        ...
        """
        if file_:
            if file_ != self.outputFileName:
                self.outputFile(file=file_)
        ...
        for i in range(self.N):
            self.printer(str(self.vzaDegrees[i]) + ' ' + str(self.szaDegrees[i])
                         + ' ' + str(self.raaDegrees[i]) + ' ' + str(self.Isotropic[i])
                         + ' ' + str(self.Ross[i]) + ' ' + str(self.Li[i]))
            # print refl data if wanted
            if reflectance:
                self.printer(' ')
            self.printer('\n')
    ...
    def outputFile(self,file=False,close=True,mode="w"):
        ...
        returns: string of filename or False if file could not be opened
        """
        if file:
...
#                 plot(kk.vzaDegrees, LiSparse[i,:],label='LiSp')
#                 plot(kk.vzaDegrees, [i,:],label='LiDen')
#                 #plot(kk.vzaDegrees,Roujean[i,:],label='Roujean')
#                 plot(kk.vzaDegrees, LiTransit[i,:],label='LiTrans')
#                 axis([-90.0,90.0,-3.0,3.0])
#                 legend(loc='best', fancybox=True)
#                 savefig(str(sza) + '.png', dpi=200, bbox_inches='tight')
#                 show()
#         except ImportError:
