...
def ComputeInterpolation(LineToProcess, NumberOfCols):
    ...
    for j in range(0,NumberOfCols):
        ...
        NSamples_p = NSamples[i,j,:]
        GoodnessOfFit_p = GoF[i,j,:]
        # Find elements to interpolate
        # only when the element is surrounded 
        # by elements with NSamples ge 7
        InterpolationPerformed = False
        for k in range(3,BandCount-3):
             if ( NSamples_p[k] < 7 or GoodnessOfFit_p[k] <= 0.0 ) and \
                ( ( NSamples_p[k-3:k] >= 7 ).any() and ( GoodnessOfFit_p[k-3:k] <> 0.0 ).any() ) and \
                ( ( NSamples_p[k+1:k+4] >= 7 ).any() and ( GoodnessOfFit_p[k+1:k+4] <> 0.0 ).any() ) :
...
# Get NSamples data
# -----------------
print 'Loading NSamples data...'
NSamples_file = sys.argv[2]

try:
   print NSamples_file
   NSamplesDataset = gdal.Open( NSamples_file, GA_ReadOnly )
   rows, cols, BandCount = GetDimensions(NSamples_file)
except:
   print "Error:", sys.exc_info()[0]
   exit(-1)
...
# Create empty array
NSamples = numpy.zeros((rows,cols,BandCount), numpy.int16)
# Populate array
for band in range(BandCount):
    NSamples[:,:,band] = NSamplesDataset.GetRasterBand(band+1).ReadAsArray()

NSamplesDataset = None

# -------------------
# Get Goodness of Fit
# -------------------
print 'Loading Goodness of Fit data...'
GoF_file = sys.argv[3]

try:
   print GoF_file
   GoFDataset = gdal.Open( GoF_file, GA_ReadOnly )
   rows, cols, BandCount = GetDimensions(GoF_file)
except:
   print "Error:", sys.exc_info()[0]
   exit(-1)
...
# Create empty array
GoF = numpy.zeros((rows,cols,BandCount), numpy.int16)
# Populate array
for band in range(BandCount):
    GoF[:,:,band] = GoFDataset.GetRasterBand(band+1).ReadAsArray()
