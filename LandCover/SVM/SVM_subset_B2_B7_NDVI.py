...
def DoClassification( LineToProcess, iCol, eCol ):
    ...
    for j in range( iCol, eCol + 1 ):
        profile = np.hstack( (NIR[:,i,j], SWIR[:,i,j], NDVI[:,i,j] ) )
    ...
    for lc_class in classes:
        ...
        for i in range ( len ( indices[0] ) ):
            ...
            if X.shape[0] == 0:
                ...
                X = np.hstack ( ( NIR_profile, SWIR_profile, NDVI_profile ) )
                y = np.array( [ lc_class ] )
            else:
                #X = np.vstack ( ( X , profile ) )
                X = np.vstack ( ( X , 
                     np.hstack ( ( NIR_profile , SWIR_profile, NDVI_profile ) ) ) )
