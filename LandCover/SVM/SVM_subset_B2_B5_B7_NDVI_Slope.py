...
def DoClassification( LineToProcess, iCol, eCol ):
    ...
    for j in range( iCol, eCol + 1 ):
        profile = np.hstack( (NIR[:,i,j], IR[:,i,j], SWIR[:,i,j], NDVI[:,i,j], DEM[i,j] ) )
        if profile.nonzero()[0].shape[0] >= 30 and DEM[i,j] < 0.0 :
...
IR_fname = sys.argv[3]
dataset = gdal.Open( IR_fname )
IR = dataset.ReadAsArray()
IR = np.ma.masked_where( IR == 0., IR )

SWIR_fname = sys.argv[4]
...
# Get DEM
DEM_fname = sys.argv[5]
dataset = gdal.Open( DEM_fname )
DEM = dataset.ReadAsArray() * -1.0
...
# ============================
if len ( sys.argv ) == 8:
    SVM_clf = sys.argv[7] # pkl file, e.g. MELODIES_SVM_clf.pkl
    ...
    for lc_class in classes:
        ...
        for i in range ( len ( indices[0] ) ):
            ...
            IR_profile = IR[:,row,col]
            SWIR_profile = SWIR[:,row,col]
            NDVI_profile = NDVI[:,row,col]
            DEM_value = DEM[row,col]
            if X.shape[0] == 0:
                #X = np.column_stack ( profile )
                X = np.hstack ( ( NIR_profile, IR_profile, SWIR_profile, NDVI_profile, DEM_value ) )
                y = np.array( [ lc_class ] )
            else:
                #X = np.vstack ( ( X , profile ) )
                X = np.vstack ( ( X , 
                     np.hstack ( ( NIR_profile , IR_profile, SWIR_profile, NDVI_profile, DEM_value ) ) ) )
