...
def DoClassification( LineToProcess, iCol, eCol ):
    ...
    for j in range( iCol, eCol + 1 ):
        profile = np.hstack( (NIR[:,i,j], SWIR[:,i,j], NDVI[:,i,j], DEM[i,j] ) )
        if profile.nonzero()[0].shape[0] >= 60:
            classification[i,j] = clf.predict ( profile )[0]
            probability[i,j,:] = clf.decision_function ( profile )[0]
...
# ============================
if len ( sys.argv ) == 7:
    SVM_clf = sys.argv[6] # pkl file, e.g. MELODIES_SVM_clf.pkl
    ...
    for lc_class in classes:
        ...
        for i in range ( len ( indices[0] ) ):
            ...
            if X.shape[0] == 0:
                ...
                X = np.hstack ( ( NIR_profile, SWIR_profile, NDVI_profile, DEM_value ) )
                y = np.array( [ lc_class ] )
            else:
                #X = np.vstack ( ( X , profile ) )
                X = np.vstack ( ( X , 
                     np.hstack ( ( NIR_profile , SWIR_profile, NDVI_profile, DEM_value ) ) ) )
                y = np.append( y , lc_class )

    #ipshell = embed()

    # Create the SVM Linear SVC classifier
    ...
    joblib.dump(clf, 'MELODIES_SVM_LinearSVC__clf.pkl') 
...
# Most probable class
new_dataset = driver.Create( 'LCC_LinearSVC_subset.tif', NIR.shape[2], NIR.shape[1], 1, gdal.GDT_Byte )
new_dataset.GetRasterBand(1).WriteArray(classification)
new_dataset = None

# Probability
new_dataset = driver.Create( 'LCP_LinearSVC_subset.tif', NIR.shape[2], NIR.shape[1], classes.shape[0], gdal.GDT_Float32 )
for band in range ( classes.shape[0] ):
    normalizedProbability = probability[:,:,band] / probability[:,:,band].max()
    new_dataset.GetRasterBand(band+1).WriteArray( normalizedProbability )
