...
LC_samples = dataset.GetRasterBand(1).ReadAsArray()
#LC_samples = dataset.GetRasterBand(2).ReadAsArray()
...
datadir = '/home/dn907640/MELODIES/data/MODIS/processing/mosaics/' + str(year) + '/interpolated'
b = gdal.Open( datadir + '/' + str(year) + '.only.f0.b' + band + '.interpolated.vrt').ReadAsArray()
...
#for lc_class in [9]:
    ...
    for i in range ( indices[0].shape[0] ):
        ...
        plot_date( GregDoY, profile , '-', lw = 0.1 )
    ...
    fill_3sigmas = fill_between( GregDoY,
                  mean_lc - (std_lc*3),
                  mean_lc + (std_lc*3),
                  alpha=0.25, edgecolor='black',
                  facecolor='black',
                  linewidth=0.15)
    fill_3sigmas.set_zorder( i + 2 )
    ...
    savefig ( "Class_" + str(lc_class) + ".band" + band + ".interpol.png", dpi = 200, bbox_inches='tight' )
    close()
