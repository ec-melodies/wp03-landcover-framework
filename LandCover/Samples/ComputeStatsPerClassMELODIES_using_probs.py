...
landCoverProbsFile = sys.argv[3]
lcp = gdal.Open( landCoverProbsFile , GA_ReadOnly ).ReadAsArray()
...
f = open( 'MELODIES_LC_stats_using_probs.txt', 'w' )
...
for lc_class in classes:
    if ( lc_class == 6 ) or \
       ( lc_class == 7 ) or \
       ( lc_class == 9 ) or \
       ( ( lc_class >= 14 ) and ( lc_class <= 21 ) ) :
        i = i + 1

    # Land cover class
    classArea[i,0] = lc_class
    # Area
    classArea[i,1] = lcp[i, indices[0], indices[1]].sum() * hectarsPerPixel
    # Percent
